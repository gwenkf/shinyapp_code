...
RaceAge = read.csv("STdrugalcdeath_raceage1599.csv", stringsAsFactors = FALSE, header = FALSE, col.names = c("State",
                  "State_code","Year","Year_code", "Race","Age_grps","Age_grpsCode","DrugAlc_induced_causes","DrugAlc_induced_code",
...
GenderAge = read.csv("STdrugalcdeath_genderage1599.csv", stringsAsFactors = FALSE, header = FALSE,
                     col.names = c("State","State_code","Year","Year_code", "Gender","Gender_code", 
                      "Age_grps","Age_grpsCode","DrugAlc_induced_causes","DrugAlc_induced_code","Deaths","Population",
                      "Crude_rate","Rate_LowerConfInt","Rate_UpperConfInt","Rate_StErr"))
...
RaceGender = read.csv("STdrugalcdeath_racegender1599.csv", stringsAsFactors = FALSE, header = FALSE, 
                      col.names = c("State","State_code","Year","Year_code", "Gender","Gender_code", 
                      "Race","DrugAlc_induced_causes","DrugAlc_induced_code","Deaths","Population","Crude_rate",
                      "Rate_LowerConfInt","Rate_UpperConfInt","Rate_StErr"))
...
#GRAPHS
deathsTotalYr = Totals_cause[DrugAlc_induced_causes == "Total"&Year != "", .(avgdeaths = mean(sum(as.numeric(Deaths))),sumdeaths = sum(as.numeric(Deaths)), 
                annual_rate = (sum(as.numeric(Deaths))/(sum(as.numeric(Population)))*1000)/365), by = .(Year)]
...
(ggplot(data=deaths_Race, aes(x=Year, y=rate, group=Race, color=Race)) + geom_point(size=1.5) + geom_line(size=1) + 
    scale_color_brewer(palette = "Dark2") + ylab("Death Rate") + theme_base())
...
Line1 <- gvisColumnChart(raceRates_Yr, xvar="Year", yvar=c("American Indian or Alaska Native", "Asian or Pacific Islander", 
...
#MAP
byST_annualrate = Totals_cause[, .(DeathsPrct = ((sum(as.numeric(Deaths))/(sum(as.numeric(Population)))*1000)/365)*100), by = .(State, Year)]
